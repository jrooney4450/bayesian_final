...
def ZipBasisRegression(df):
    ...
    #Guess Parameters: #best case would grid search
    #sdlat = 0.01
    sdlat = np.logspace(.0001,.1,10)
    sdlong = np.logspace(.0001,.1,10)
    #sdlong = 0.05
    ...
    for lat_ in range(len(sdlat)):
        ...
        #rest are 2D gaussians centered on NYC zip codes
            ...
            try:
                #IF this phi was already made just load it in
            ...
            #Use approximate alpha and beta tuning since M = 334 and N = 16000 so M~<<N to save time
    ...
    ax.scatter(training.longs,training.lats, s=0.8, c='black',alpha = 0.1)
...
def LatLongBasisRegression(df):
    ...
    #summary results are comparable, sorry for sparse commenting here
    conclat = 15
    conclong = 15
